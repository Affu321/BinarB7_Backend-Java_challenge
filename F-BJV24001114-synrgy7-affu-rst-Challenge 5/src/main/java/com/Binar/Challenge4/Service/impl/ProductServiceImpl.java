...
import com.Binar.Challenge4.Service.ProductService;
...
public class ProductServiceImpl implements ProductService {
    ...
    public Map addProduct(Product product) {
        try {
            ...
            Optional<Merchant> chekMerchant = Optional.ofNullable(merchantRepository.getByIdMerchant(doSave.getMerchant().getId()));
            Optional<Product> existingProduct = Optional.ofNullable(productRepository.getByIdProduct(doSave.getId()));

            existingProduct.get().setMerchant(chekMerchant.get());
            return response.sucsess(productRepository.save(product));
        } catch (Exception e) {
            response.eror("Error saving product", e);
